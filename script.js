...
let typeThis = document.getElementById("typeThis");
let typeHere = document.getElementById("typeHere");
let typeThis_list = []
let sentence = ""
...
function loadSentence(){
    for (let i = 50; i >= 0; i--){
        typeThis_list.push(keywords[Math.floor(Math.random() * 900)])
    }
    for (item of typeThis_list){
        sentence += (item) + " "
    }
    sentence = sentence.slice(0, -1) // Removes space added to LAST
    typeThis.innerText = sentence
    console.log(typeThis_list.length, typeThis_list)
}


typeHere.addEventListener("focus", function(){
    console.log("div is focused"); // Verification that div is working
    typeHere.addEventListener("keydown", function(event){
        const key = event.key;
        event.preventDefault(); // Blocks default keyabord buttons input

        if (/^[a-zA-Z0-9]$/.test(key) || key == " " || key == "Backspace"){
            if (typeHere.innerText != "" && key == " " && spaceInserted != true && typeThis_list[currentWordIndex].slice(-1) == typedWord){
            ...
            if (typeThis_list[currentWordIndex][currentCharIndex] == key){
        		// console.log(totypeList[now][now_now] == key);
			    	// typehere.innerHTML += totypeList[now][now_now]
			    	// Insert the character at the cursor's position
			    	 document.execCommand('insertText', false, typeThis_list[currentWordIndex][currentCharIndex]);
			    	 spaceInserted = false
			    	console.log(true)
	        		...
	        		// blink()
	        		// setTimeout(function() {
	        		// 	document.getElementById('wrong').classList.remove("wrong")
	        		// }, 5000);
                    // alert("Wrong")
                    console.log("WRong")
